...
class BundleSermonScreen extends Component {
  ...
  componentDidMount = () => {
    ...
    this.title = this.props.route.params.bundle_title
    this.preacher = this.props.route.params.preacher
    ...
    this.sermon = [];
    this.SetContentAndSession(this.bundle_id)
              ...
              <TabView
                ...
                <Tab title="Sermon">
                  <View style={{ paddingLeft: 5, paddingTop: 5 }}>
                    <Text >TOTAL : {this.total_session} Sermon</Text>
  ...
  async SetContentAndSession(bundle_id) {
    this.content = []
    await fetch("https://nepho.id/pondokpujian/api/sermon.php?sermon_id=" + bundle_id)
      .then((response) => response.json())
      .then(async (responseJson) => {

        let lc = responseJson;
        await fetch("https://nepho.id/pondokpujian/api/sermon.php?bundle_id=" + bundle_id)
          .then((response) => response.json())
          .then((responseJson) => {
            let ls = responseJson;
            for (let i = 0; i < ls.length; i++)
              this.content.push(
                <TouchableOpacity style={{ padding: 5 }} key={Math.random()} onLongPress={() => {
                  ...
                  <View style={{ flex: 1, flexDirection: "row", marginHorizontal: 10, marginBottom: 0 }}>
                    <Text style={{ flex: 8 }} category="subtitle-s-regular" status="highempha">{(i + 1).toString()}. {ls[i].sermon_title.replace((i+1).toString(), "")}</Text>
                    <View style={{ flex: 2 }}>
                      <Text style={{ textAlign: "right", color: "gray" }} category="label-m-regular" status="lowempha">{ls[i].duration} m</Text>
                    </View>
                  </View>
                  <Text category="p2" style={{marginLeft:30}}>{this.preacher}</Text>
...
export default BundleSermonScreen
